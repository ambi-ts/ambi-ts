...
describe(">>> Canvas", () => {
    ...
    describe(">> API", () => {
        ...
        it("should set css style", () => {
            const zIndex = "1";
            expect(canvas.element.style.zIndex).not.toBe(zIndex);
            canvas.setStyle({ zIndex });
            expect(canvas.element.style.zIndex).toBe(zIndex);
        });
        describe(">>> calculate local point by global", () => {
            ...
            it("should return local point otherwise", () => {
                expect(canvas.calcLocalPointFrom(new Vector2(200, 300))).toEqual(new Vector2(180, 280));
            });
        });
    });
});
